...
### Deployed (live) — 2026-07-03
- **Frontend (Vercel):** https://agentic-research-assistant-nine.vercel.app — root dir `frontend/`; env `NEXT_PUBLIC_API_URL` set to the Render backend URL (inlined at build time).
- **Backend (Render):** https://agentic-research-assistant-jt94.onrender.com — root dir `backend/`; free tier (sleeps when idle → slow cold start); start cmd `uvicorn main:app --host 0.0.0.0 --port $PORT` (`backend/Procfile`); Python pinned to 3.12 (`backend/.python-version`) since hosts don't support 3.14; all API keys + `FRONTEND_ORIGINS`=the Vercel URL set as env vars in the Render dashboard.
- **Auto-deploy:** pushing to `main` redeploys both. CORS is driven by `FRONTEND_ORIGINS` (comma-separated origins).
...
- **App name "Sourced" (2026-07-03):** display/brand name is **Sourced** (nav, hero, tab title); the repo/folder stays `agentic-research-assistant`. `research.ai` was a placeholder and was dropped.
- **UI = developer-dark, Vercel-flavored (2026-07-03):** class-based dark mode forced via `.dark` on `<html>` + `@custom-variant` in globals.css; flat black, Geist font, white buttons, gradient headline. FONT BUG FIXED: `var(--font-sans)` doesn't resolve under Tailwind v4 `@theme inline`, which silently fell back to Times serif — reference `var(--font-geist-sans)` directly.
- **Deploy: Vercel + Render (2026-07-03):** two separate deploys from the monorepo (root dirs `frontend/` and `backend/`). No code change for prod — CORS reads `FRONTEND_ORIGINS` and the frontend reads `NEXT_PUBLIC_API_URL`, both set as host env vars. Python pinned to 3.12 for the backend host.
...
- **App renamed "Sourced" → "Docent" (2026-07-04):** brand/display name is now **Docent** everywhere (nav, hero, tab title, favicon). Supersedes the 2026-07-03 "Sourced" decision. Repo/folder stays `agentic-research-assistant`.
- **UI redesigned to the "Docent" design system (2026-07-04):** warm sage/clay palette (light `--bg:#edefe8` / clay `#2f6a4c`; dark `--bg:#0f1210` / sage `#5faa7f`), fonts Inter (UI) + Newsreader (serif prose/headings) + Geist Mono (labels/scores). Theme via `[data-theme]` on `<html>` (default light) with a pre-paint inline script (no FOUC) and a `useSyncExternalStore`-backed `useTheme()` synced across toggles by a `theme-changed` event. Replaces the earlier developer-dark theme. NOTE: `@theme inline` does NOT emit `--font-sans` as a real CSS var — reference `--font-inter` directly in raw CSS.
- **Chat-first layout + landing/app split (2026-07-04):** `Workspace` is a two-pane shell (266px `Sidebar` + main). Both `AskAssistant` (chat) and `SemanticSearch` ("Search chunks") stay mounted, toggled by `display`, so streaming state survives view switches. Cross-component signals via window events: `documents-changed`, `new-thread`, `theme-changed`. Landing page lives at `/` (marketing hero, CTA → `/app`); the app moved to `/app`. Removed the fake "Alex Rivera" placeholder identity (sidebar footer + chat avatar) — that footer is where a real user menu goes once auth lands.
- **Docent logo mark (2026-07-04):** `Brand.tsx` exports `DocentMark` — a rounded "D" drawn as one outline (stem = closing edge) with two text lines + a clay folded corner. Reused for the chat assistant avatar and mirrored in the favicon (`src/app/icon.svg`, replacing the default Next favicon). Wordmark is Inter 600.
- **Dev proxy for CORS-free local backend (2026-07-04):** local dev has no `backend/.env` keys, and Render's CORS only allows the Vercel origin, so `localhost:3000` can't call Render directly. `next.config.ts` reverse-proxies `/proxy/*` → `BACKEND_PROXY_TARGET` (server-to-server, no CORS) when that env is set; `.env.local` sets `NEXT_PUBLIC_API_URL=/proxy` + `BACKEND_PROXY_TARGET=<render url>`. Prod leaves `BACKEND_PROXY_TARGET` unset → the rewrite is a no-op and the frontend calls Render directly. Caveat: local uploads/deletes hit the live Pinecone data.
- **Anthropic client hardening (2026-07-04):** `rag._get_client()` now sets `timeout=45.0, max_retries=2`. Without a timeout the SDK waits its 10-min default on a stuck call, which surfaced as `/ask/agentic` returning HTTP 200 then streaming zero bytes; retries also absorb transient `RemoteDisconnected` drops (seen on Render).
- **Frontend UX: dead controls made live (2026-07-04):** example chips now submit the question (`handleAsk(ex)`) instead of only filling the box; "New thread" clears the transcript AND refocuses the input; internal nav uses `next/link` (logo → `/`, CTAs → `/app`). Root `README.md` added.
- **Google auth via Auth.js / NextAuth v5 (2026-07-04):** `next-auth@5.0.0-beta.31` with the Google provider, JWT sessions (no DB — session in an encrypted cookie, fits the no-DB design). `src/auth.ts` exports `authEnabled` = both `AUTH_GOOGLE_ID` + `AUTH_GOOGLE_SECRET` present. GRACEFUL: when unset, auth is fully dormant — `/app` is open to everyone and no user menu shows (the app behaves exactly as before). When set, `/app` (a Server Component) calls `auth()` and redirects to `/` if unauthenticated; the landing CTA becomes "Continue with Google"; the sidebar footer shows the user + sign-out. Gating is done in the Server Component (NOT the proxy) to sidestep Next 16's `middleware.ts`→`proxy.ts` rename. Route handler at `src/app/api/auth/[...nextauth]/route.ts`; sign-in/out are server actions in `src/app/actions/auth.ts`. Env (server-side only): `AUTH_SECRET`, `AUTH_GOOGLE_ID`, `AUTH_GOOGLE_SECRET` — placeholders documented in `frontend/.env.local`. Backend JWT verification (protecting the FastAPI endpoints) is a deliberate follow-up, NOT yet done — today auth only gates the frontend `/app`.
- **Next 16 gotcha — middleware is now `proxy.ts` (2026-07-04):** confirmed via `node_modules/next/dist/docs`. Avoided it entirely by gating in a Server Component. Our `/proxy/*` dev backend proxy is a `next.config.ts` rewrite, unrelated to the `proxy.ts` file convention.
- **Local dev proxy is now a route handler, not a rewrite (2026-07-05):** the `next.config.ts` `rewrites()` proxy BUFFERED SSE, so streamed answers arrived all-at-once locally. Replaced with `src/app/proxy/[...path]/route.ts` that pipes the backend response body straight through (`x-accel-buffering: no`) → tokens stream incrementally. It also injects `X-API-Key` from server env (see backend hardening). Prod is unaffected (no proxy; calls Render directly, which already streams).
- **Backend hardening — rate limiting + optional API-key gate (2026-07-05):** `main.py` uses `slowapi` for per-IP rate limits on the expensive/mutating endpoints (`/upload`, `/search`, `/ask`, `/ask/stream`, `/ask/agentic`, DELETE `/documents`) — generous env-tunable defaults (`RATE_LIMIT_*` in `config.py`); the key func reads `X-Forwarded-For` so it's per-user behind Render's proxy. A `require_api_key` dependency gates those same endpoints behind `X-API-Key` ONLY when `BACKEND_API_SECRET` is set (graceful: open by default, so the live deploy is unchanged). The Next proxy injects the secret server-side so it never reaches the browser. To actually turn the gate on in prod: set `BACKEND_API_SECRET` on Render AND route prod through the proxy (`NEXT_PUBLIC_API_URL=/proxy` + `BACKEND_PROXY_TARGET` on Vercel). `/health` + `/status` stay open. Backend JWT/session verification (tying the gate to a signed-in user) is still a later option; today it's a shared secret.
- **Tavily retry + degrade (2026-07-05):** `web_search.search_web` retries transient `RemoteDisconnected`/`Connection aborted` (Tavily runs on `requests`, which isn't covered by the Anthropic client's httpx retries) up to 3 tries, then returns `[]` so a web-search hiccup degrades to documents-only instead of aborting the whole answer.
- **Backend tests (2026-07-05):** `pytest` suite (`backend/tests/` + `conftest.py`) — chunking (sizes/overlap/edge cases), config placeholder + `api_auth_enabled` detection, and API endpoints via `TestClient`: health/status shape, upload non-PDF → 400, the auth gate (open when unconfigured; 401 without/with wrong key; passes with the right key), and rate limiting → 429. Runs with NO external keys (protected endpoints hit their 503 branch). Deps `slowapi` + `pytest` added; run `python -m pytest` from `backend/`.
...
- GitHub: `gh` authed as `sean-kim05`. Repo `sean-kim05/agentic-research-assistant` (public). Push to a remote as we go (see Decisions re: no Claude attribution in commits).
- **Deployed live (2026-07-03):** see the "Deployed (live)" block in Current State for URLs + setup. Free Render tier sleeps → first request is slow. Prod API keys can be burned by anyone with the URL — consider provider usage caps for a public demo.
- **Transitive LangChain packages:** the Render build pulled in `langchain-core`/`langchain-text-splitters`/`langsmith` etc. as TRANSITIVE deps (via `tavily-python`). Our code does NOT import or use LangChain — the "plain Python, no LangChain" rule holds; they're just along for the ride.
- **UI is intentionally rough / to be redesigned:** Sean plans to redo the frontend from a mockup. Backend + data flow are the finished, interview-relevant parts.
